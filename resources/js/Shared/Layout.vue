<template>
  <div class="min-h-screen bg-gradient-to-br from-amber-50 to-gray-100">
    <div id="dropdown" />
    <div class="md:flex md:flex-col">
      <div class="md:flex md:flex-col md:h-screen">
        <div class="md:flex md:shrink-0">
          <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-amber-600 to-amber-800 md:shrink-0 md:justify-center md:w-64 shadow-lg">
            <div class="flex items-center space-x-4">
              <Link href="/" class="flex items-center group transition-transform duration-300 hover:scale-105">
                ...
                <span class="text-white text-xl font-bold ml-3 group-hover:text-amber-100">
            ...
            <dropdown class="md:hidden" placement="bottom-end">
              <template #default>
                <button class="text-white hover:text-amber-100 focus:outline-none transition-colors duration-200">
          ...
          <div class="md:text-md flex items-center justify-between p-4 w-full bg-white border-b shadow-sm md:px-12 md:py-0">
            <div class="mr-4 mt-1 text-amber-900 font-medium">{{ auth.user.account.name }}</div>
              ...
              <dropdown class="mt-1" placement="bottom-end">
                <template #default>
                  <div class="group flex items-center cursor-pointer select-none">
                    <div class="mr-1 text-gray-700 group-hover:text-amber-600 transition-colors duration-200">
                      <span>{{ auth.user.first_name }}</span>
                      <span class="hidden md:inline">&nbsp;{{ auth.user.last_name }}</span>
                    </div>
                    <icon class="w-5 h-5 fill-gray-700 group-hover:fill-amber-600 transition-colors duration-200" name="cheveron-down" />
                  </div>
                </template>
                <template #dropdown>
                  <div class="mt-2 py-2 bg-white rounded-lg shadow-xl border border-gray-100">
                    <Link class="block px-6 py-2 hover:bg-amber-50 hover:text-amber-700 transition-colors duration-200" 
                    ...
                    <Link class="block px-6 py-2 w-full text-left hover:bg-amber-50 hover:text-amber-700 transition-colors duration-200" 
                          href="/logout" 
                          method="delete" 
                          as="button">
        ...
        <div class="md:flex md:grow md:overflow-hidden">
          <div class="hidden shrink-0 p-6 w-64 bg-gradient-to-b from-amber-800 to-amber-900 overflow-y-auto md:block">
            <main-menu />
          </div>
